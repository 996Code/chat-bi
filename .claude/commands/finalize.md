...
项目专属的完整闭环流程：代码检查 → 需求追溯更新 → 补全测试 → 运行测试 → 补全文档 → 提交推送。
...
**自动交叉验证：从需求文档提取 ❌ Missing 条目关键词，去代码里搜是否存在。**

```bash
# 1. 列出所有 ❌ Missing 条目（自动发现，不写死）
grep "❌ Missing" doc/需求文档.md
```

**对每个 ❌ Missing 条目，从其需求描述中提取 2-3 个核心关键词，在代码中搜索：**

```bash
# 示例：从文档行提取关键词并搜索（通用逻辑，不依赖具体条目）
# 关键词来源：需求描述中的中文术语 + 英文需求 ID 前缀
# 搜索范围：backend/app/ + frontend/src/
# 判定：关键词在代码中出现 3 次以上 → 很可能已实现，需要人工确认
```

**检测逻辑（LLM 执行，非脚本）：**
1. 读取 `doc/需求文档.md` 所有 ❌ Missing 行
2. 对每条，从需求描述文本中提取关键词（如"语义缓存" → `semantic_cache`、`语义缓存`）
3. 在 `backend/app/` 和 `frontend/src/` 中 grep 关键词
4. 有命中 → 检查对应代码是否完整（函数/端点/组件），完整则标记 ✅ Done
5. 无命中 → 确认 Missing
6. 更新 Coverage 统计
...
```
[1/7] 代码质量检查 — ✅ 无问题
[2/7] 需求追溯 — ✅ 7 个条目更新为 Done
[3/7] SQL 管线 — ✅ 12 个模块 OK
[4/7] 测试补全 — +5 个新测试用例
[5/7] 全部测试 — ✅ 197 passed
[6/7] 文档补全 — ✅ 开发记录 + 项目状态 + 路线图 已更新
