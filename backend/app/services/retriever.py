...
async def _llm_refine(
    ...
    prompt = (
        ...
        f"重要规则:\n"
        f"1. 问题涉及多张表时必须全部选出。例如'售后处理时长'涉及售后表+售后日志表,'搜索转化率'涉及搜索表+推荐表。漏选会导致下游无法生成正确 JOIN。\n"
        f"2. 宁缺毋滥: 若无真正匹配, 返回空数组 (在 BI 场景, 返回错误数据比返回无数据更危险)。\n"
        f"3. 不要只选 score 最高的, 要选问题语义涉及的所有表。\n"
        f"只返回 JSON, 格式: "
