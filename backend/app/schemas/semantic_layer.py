...
# F9: 注入防御正则 (供 API 层校验用, schema 层不加校验以兼容已有数据)
_METRIC_DANGEROUS = _re.compile(
    r";|--|/\*|\*/",
    _re.IGNORECASE,
)
...
class Metric(BaseModel):
    ...
    F9 注入防御策略:
      - schema 层不加校验 (兼容已有数据/LLM 生成/规则推断)
      - API 层 (semantic_models.py) 校验用户提交的 formula/condition
      - 执行层 (sql_validator.py) 三层 AST 拦截是终极防线
    """
    model_config = ConfigDict(extra="forbid")
