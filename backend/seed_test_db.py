"""
Seed test database on 192.168.3.110: chatbi_test
Rich e-commerce business model with 15 tables and realistic test data.

Usage:
  SEED_DB_HOST=192.168.3.110 SEED_DB_PASS=yjt_mysql .venv/bin/python seed_test_db.py
"""
import asyncio
import os
import random
...
async def main():
    ...
    # Drop & recreate
    eng = create_async_engine(DB_URL_ROOT)
    async with eng.begin() as conn:
        await conn.execute(text('DROP DATABASE IF EXISTS chatbi_test'))
        await conn.execute(text('CREATE DATABASE chatbi_test DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci'))
    await eng.dispose()
    print('Database recreated: chatbi_test')
    ...
    async with db.begin() as conn:
        ...
        # ===== SEED DATA =====
        user_ids = await _seed_users(conn, base_time)
        print(f'  Users: 500')

        await _seed_addresses(conn, user_ids)

        brand_ids = await _seed_brands(conn)
        print(f'  Brands: {len(BRANDS)}')

        cat_ids = await _seed_categories(conn)
        print(f'  Categories: {len(cat_ids)}')

        product_ids = await _seed_products(conn, cat_ids, brand_ids)
        print(f'  Products: {len(product_ids)}')

        order_ids, order_user_ids = await _seed_orders(conn, user_ids, base_time)
        print(f'  Orders: {len(order_ids)}')

        item_count = await _seed_order_items(conn, order_ids, order_user_ids)
        print(f'  Order Items: {item_count}')

        await _seed_payments(conn, order_ids[:1600], base_time)
        print(f'  Payments: 1600')

        await _seed_shipping(conn, order_ids[:1200], base_time)
        print(f'  Shipping: 1200')

        review_count = await _seed_reviews(conn, order_ids, user_ids, base_time)
        print(f'  Reviews: {review_count}')

        await _seed_returns(conn, order_ids, user_ids, base_time)
        print(f'  Returns: 150')

        await _seed_coupons(conn, user_ids, base_time)

        wh_ids = await _seed_warehouses(conn)
        print(f'  Warehouses: {len(wh_ids)}')

        inv_count = await _seed_inventory(conn, product_ids, wh_ids)
        print(f'  Inventory: {inv_count}')

    await db.dispose()
    print('\nchatbi_test seeded successfully!')
    ...
    TABLES = [
        ...
        ('t_reviews', '''
            CREATE TABLE t_reviews (
                ...
                images INT DEFAULT 0 COMMENT '评价图片数',
        ...
        ('t_returns', '''
            CREATE TABLE t_returns (
                ...
                status VARCHAR(20) DEFAULT 'pending' COMMENT 'pending/approved/shipped/completed/rejected',
                carrier VARCHAR(100),
                tracking_no VARCHAR(100),
                created_at DATETIME NOT NULL DEFAULT NOW(),
...
async def _seed_addresses(conn, user_ids):
    districts = ['朝阳区', '海淀区', '浦东新区', '黄浦区', '天河区', '福田区',
                 '西湖区', '武侯区', '江岸区', '玄武区', '渝中区', '雁塔区',
                 '芙蓉区', '金水区', '和平区', '工业园区', '市南区', '沙河口区', '思明区', '鼓楼区']
    for uid in user_ids[:300]:
        u = await conn.execute(text("SELECT city, province, username FROM t_users WHERE id = :id"), {'id': uid})
        row = u.fetchone()
        city, province, uname = row
        district = random.choice(districts)
        for j in range(random.randint(1, 3)):
            detail = random.choice(['路', '街', '大道', '巷']) + f'{random.randint(1,999)}号'
            detail += random.choice(['', ' 小区', ' 栋', f'{random.randint(1,30)}单元{random.randint(101,2802)}室'])
            await conn.execute(
                text("INSERT INTO t_addresses (user_id, name, phone, province, city, district, detail, is_default) "
                     "VALUES (:uid, :n, :p, :pr, :c, :d, :dt, :df)"),
                {'uid': uid, 'n': uname,
                 'p': f'1{random.randint(3,9)}{random.randint(100000000,999999999)}',
                 'pr': province or '', 'c': city, 'd': district, 'dt': detail, 'df': 1 if j == 0 else 0})


async def _seed_brands(conn):
    ids = []
    for b in BRANDS:
...
async def _seed_categories(conn):
    ...
    for parent_name, children in CATEGORIES:
        ...
        for j, child in enumerate(children):
            ...
            ids[child] = r.lastrowid
    return ids


async def _seed_products(conn, cat_ids, brand_ids):
    brand_mapping = {
    ...
    for cat_name, items in PRODUCT_DATA.items():
        ...
        for name, price, cost in items:
            ...
            tags = random.sample(['新品', '热销', '特价', '限时折扣', '包邮', '官方正品', '会员专享'],
    ...
    for name, price, cost in DEFAULT_PRODUCTS:
        ...
        r = await conn.execute(
            ...
            {'n': name, 'c': cid, 'p': price, 'cp': cost,
             'mp': round(price * 1.5, 2), 's': random.randint(100, 500),
             'sk': f'SKU{sku:06d}'})
        ids.append(r.lastrowid)
        sku += 1
    return ids
...
async def _seed_orders(conn, user_ids, base_time):
    ids = []
    user_id_list = []
    for i in range(2000):
        ...
        city = random.choice(CITIES)
        province = PROVINCES.get(city, '')
        note = random.choice(['', '', '', '', '请尽快发货', '周末不在家请放门口', '送前电话联系', '需要发票'])
    ...
    return ids, user_id_list


async def _seed_order_items(conn, order_ids, order_user_ids):
    # First get all products
    pr = await conn.execute(text("SELECT id, name, price FROM t_products"))
    products = [(r[0], r[1], float(r[2])) for r in pr.fetchall()]
    count = 0
    for oid_idx, oid in enumerate(order_ids):
        ...
        for pid, pname, price in picked:
            ...
            await conn.execute(
                ...
                {'oid': oid, 'pid': pid, 'pn': pname, 'q': qty, 'p': price, 'sub': subtotal})
            count += 1
    return count


async def _seed_payments(conn, order_ids, base_time):
...
async def _seed_shipping(conn, order_ids, base_time):
    for oid in order_ids:
...
async def _seed_reviews(conn, order_ids, user_ids, base_time):
    review_contents = [
        '质量很好，非常满意', '物流很快，包装完好', '性价比很高，推荐购买', '物超所值',
        '一般般吧，没有描述的那么好', '价格便宜，质量还行', '第二次购买了，一如既往的好',
        '颜色和图片有差异', '尺寸不合适，需要退换', '', '超值！', '客服态度很好',
        '用了几天才来评价，确实不错', '包装精美，送人很合适', '物流太慢了',
        '比实体店便宜很多', '孩子很喜欢', '手感不错，质感很好', '会回购的',
        '这个价位无敌了', '外观设计很棒，细节处理到位', '有点小瑕疵但不影响使用',
        '发货速度超快，隔天就到了', '品质一如既往的好', '给父母买的，他们说很好用',
    ]
    count = 0
    for i in random.sample(order_ids[:1200], min(800, 1200)):
        ...
        images = random.choices([0, 1, 2, 3, 4, 5], weights=[30, 25, 20, 15, 7, 3])[0]
        is_anon = random.choices([0, 1], weights=[85, 15])[0]
        helpful = random.choices([0, 1, 2, 3, 5, 10, 20], weights=[30, 25, 15, 12, 10, 5, 3])[0]
        uid = random.choice(user_ids)
        pr = await conn.execute(
            text("SELECT product_id FROM t_order_items WHERE order_id = :oid LIMIT 1"), {'oid': i})
        prow = pr.fetchone()
        pid = prow[0] if prow else random.randint(1, 150)
        await conn.execute(
            text("INSERT INTO t_reviews (order_id, product_id, user_id, rating, content, images, "
                 "is_anonymous, helpful_count, created_at) "
                 "VALUES (:oid, :pid, :uid, :r, :c, :img, :anon, :hc, :ca)"),
            {'oid': i, 'pid': pid, 'uid': uid, 'r': rating, 'c': content, 'img': images,
             'anon': is_anon, 'hc': helpful,
             'ca': base_time + timedelta(days=random.randint(1, 179))})
        count += 1
    return count


async def _seed_returns(conn, order_ids, user_ids, base_time):
    for i in random.sample(order_ids[:500], min(150, 500)):
        reason = random.choice(RETURN_REASONS)
        desc = random.choice(['', '商品收到时已损坏', '收到的和描述完全不同', '尺寸太小了', '颜色不对'])
        refund = round(random.uniform(29, 3000), 2)
        status = random.choices(['pending', 'approved', 'shipped', 'completed', 'rejected'],
                                weights=[20, 30, 15, 25, 10])[0]
        uid = random.choice(user_ids[:100])
        await conn.execute(
            text("INSERT INTO t_returns (order_id, user_id, reason, description, refund_amount, "
                 "status, carrier, tracking_no, created_at, approved_at, completed_at) "
                 "VALUES (:oid, :uid, :r, :d, :ra, :s, :c, :t, :ca, :aa, :coa)"),
            {'oid': i, 'uid': uid, 'r': reason, 'd': desc, 'ra': refund, 's': status,
...
async def _seed_coupons(conn, user_ids, base_time):
    coupon_data = [
    ...
    for code, name, dtype, value, min_ord, max_u in coupon_data:
        await conn.execute(
            ...
            {'c': code, 'n': name, 'dt': dtype, 'dv': value, 'mo': min_ord,
             'mu': max_u, 'pul': random.randint(1, 3),
             'vf': base_time, 'vu': base_time + timedelta(days=365)})

    for uid in user_ids[:200]:
        picked = random.sample(range(8), random.randint(1, 5))
        for idx in picked:
...
async def _seed_warehouses(conn):
    wh_data = [
...
async def _seed_inventory(conn, product_ids, wh_ids):
    count = 0
    for pid in product_ids:
        for wid in random.sample(wh_ids, random.randint(1, 4)):
            ...
            await conn.execute(
                ...
                {'pid': pid, 'wid': wid, 'q': qty, 'r': reserved, 'ms': min_stock})
            count += 1
    return count
