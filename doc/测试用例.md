...
> 更新时间：2026-05-05
> 执行方式：pytest 自动化集成测试 + 单元测试 + 真实数据 API 测试
...
| test_unit.py | SQL验证+登录锁定+脱敏 | 24 | 24 | 0 | ✅ |
| test_phase2.py | Phase 2 需求（语义层/缓存/图表/分页） | 20 | 20 | 0 | ✅ |
| test_phase2_extra.py | Phase 2 补充（别名/指标/计算字段/反馈） | 15 | 15 | 0 | ✅ |
| test_additional.py | 额外功能测试 | 12 | 12 | 0 | ✅ |
| test_new_features.py | 新功能+Partial补全（脱敏/SQL解释/上下文/分页/锁定降级/RBAC/审计/历史/隔离/启停） | 27 | 27 | 0 | ✅ |
| test_e2e.py | E2E 全流程 | 8 | 8 | 0 | ✅ |
| **总计** | | **126** | **126** | **0** | **✅ 全通过** |

### 真实数据 API 测试结果（MySQL 192.168.3.110:3306/chatbi）

| 测试项 | 结果 | 说明 |
|--------|------|------|
| 登录 + JWT 含 role | ✅ | admin 登录 JWT 包含 `role: "admin"` |
| Refresh token 保留 role | ✅ | 刷新后 JWT 仍包含 role |
| 审计日志 admin 访问 | ✅ | 返回 50 条记录，has_more=True |
| 审计日志 analyst 403 | ✅ | 非 admin 返回 FORBIDDEN |
| 审计日志多维度筛选 | ✅ | action=USER_LOGIN 筛选正常 |
| 查询历史搜索 | ✅ | text=订单 搜索返回 1 条结果 |
| 查询历史状态筛选 | ✅ | status=success 筛选正常 |
| 查询历史自动保存 | ✅ | 查询后历史含 success/row_count/execution_time_ms/chart_type |
| 数据源 toggle (admin) | ✅ | 切换 is_active 成功 |
| 数据源 toggle (analyst) | ✅ | 非 admin 返回 FORBIDDEN |
| NL 查询（各城市订单数量） | ✅ | SQL 正确，30 行数据，bar 图表，65ms |
| SSE 流式查询 | ✅ | 6 个事件：intent/semantics/sql/data/chart/complete |
| SQL 解释 | ✅ | 返回自然语言描述 |
| CSV 导出 | ✅ | 31 行 CSV，含 header + 30 行数据 |

### Phase 2 新增测试覆盖

| 测试文件 | 覆盖需求 | 关键测试项 |
|----------|---------|-----------|
| test_new_features.py | SEC-04, SQL-10, SQL-09, API-04, AUTH-07 | 数据脱敏、SQL 解释、多轮上下文、cursor 分页、登录锁 Redis 降级 |
| test_phase2.py | META-02~06, PERF-01, CHART-01~08, API-04~05 | 别名配置、指标定义、计算字段、Redis 缓存、图表推断、cursor 分页、CSV 导出 |
| test_phase2_extra.py | META-02~06, UX-09, API-05 | 别名/指标/计算字段完整 CRUD、反馈提交、CSV 导出 |

### SSE 流式查询测试

| 事件类型 | 测试方式 | 期望 |
|----------|---------|------|
| intent | SSE event 解析 | `intent=DataQuery` 或 `Other` |
| semantics | SSE event 解析 | Schema 选择完成 |
| sql | SSE event 解析 | 生成的 SQL 字符串 |
| data | SSE event 解析 | `{success: true, columns, rows, row_count}` |
| chart | SSE event 解析 | `{chart_type: "bar"|"line"|"pie"|...}` |
| complete | SSE event 解析 | 所有 running 步骤标记 done |
| error | SSE event 解析 | `{error: "错误信息"}` |

### 15 条复杂查询测试

| # | 查询 | 期望结果 |
|---|------|---------|
| 1 | 查询所有订单 | SELECT * FROM t_orders |
| 2 | 查询退款订单 | WHERE status = 'refunded' |
| 3 | 各城市的订单数量 | GROUP BY city |
| 4 | 月度销售额趋势 | GROUP BY MONTH + SUM |
| 5 | 支付方式分布 | GROUP BY payment_method |
| 6 | 订单金额 Top5 | ORDER BY total_amount DESC LIMIT 5 |
| 7 | 各产品类别的平均订单金额 | GROUP BY category + AVG |
| 8 | 退款率最高的城市 | 复杂聚合 + 排序 |
| 9 | 近7天的订单数量 | 时间过滤 |
| 10 | 各城市的月销售额 | 双维度 GROUP BY |
| 11 | 退款订单在各城市的月销售额 Top5 | 多条件 + 排序 + 限制 |
| 12 | 每月新增用户数 | 时间序列 + COUNT |
| 13 | 订单状态分布 | GROUP BY status |
| 14 | 各产品的销售额排名 | JOIN + GROUP BY + ORDER BY |
| 15 | 今年各月订单金额对比 | 年度过滤 + 月度聚合 |

### 已修复的 Bug

| Bug | 根因 | 修复 |
|-----|------|------|
| TENANT-02 租户隔离失败 | auth.py 复用第一个已存在的 tenant | 每个用户注册时创建独立 tenant |
| AUTH-07 登录锁定不生效 | check_lock 对 locked_until=None 误删 | 只在 locked_until 已设置且过期时删除 |
| 前端 ChatView loading 错误 | Pinia ref 在非 setup 上下文中用了 .value | 直接使用 chatStore.loading |
| SQLAlchemy 2.0 text() 缺失 | SELECT 1 未用 text() 包裹 | 添加 text() |
| datasource.py 重复赋值 | last_health_check 被赋值两次 | 删除重复行 |
| data_masking lambda 参数错误 | lambda 接收字符串却当 match 对象用 | `lambda v: v[:3] + "****" + v[-4:]` |
| SSE 解析跳过事件 | for 循环内 shift() 修改数组 | 改用 while + 索引遍历 |
| email_verified 403 拦截测试 | 注册默认 email_verified=False | 注册时默认 True |
| 本地缓存与 Redis 不一致 | FLUSHDB 只清 Redis 不清本地 | 移除本地缓存，统一 Redis |
| cursor 分页格式变更 | 旧测试期望列表格式 | 统一用 `data.get("data", data)` |
| 元数据中文描述缺失 | LLM 无法映射中文到表名 | seed.py 补全 10 表 50+ 列描述 |
